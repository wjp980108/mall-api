...
CREATE TABLE IF NOT EXISTS `t_banner` (
    ...
    `img_url_platform` VARCHAR(50)     DEFAULT NULL COMMENT '轮播图存储平台:local-1/aliyun-oss-1等',
...
CREATE TABLE IF NOT EXISTS `t_goods` (
    ...
    `goods_thumb_platform`  VARCHAR(50)     DEFAULT NULL COMMENT '商品缩略图存储平台:local-1/aliyun-oss-1等',
...
CREATE TABLE IF NOT EXISTS `t_session` (
    ...
    `rush_start_time`      TIME         NOT NULL COMMENT '每日抢购开始时间（时:分:秒，例：09:50:00）',
    `rush_end_time`        TIME         NOT NULL COMMENT '每日抢购结束时间（时:分:秒，例：17:00:00，不支持跨天）',
    ...
    `bg_img_platform`      VARCHAR(50)  DEFAULT NULL COMMENT '场次背景图存储平台:local-1/aliyun-oss-1等',
...
CREATE TABLE IF NOT EXISTS `t_consign_goods` (
    ...
    `payment_amount` DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT '付款金额(实付价格，单价口径)：管理端手工维护的固定配置值，下单/取消/转移等抢购业务流程一律不回写',
    ...
    `cover_img_platform`   VARCHAR(50)   DEFAULT NULL COMMENT '商品缩略图存储平台:local-1/aliyun-oss-1等',
    `detail_img`           VARCHAR(500)  COMMENT '商品详情图url',
    `detail_img_platform`  VARCHAR(50)   DEFAULT NULL COMMENT '商品详情图存储平台:local-1/aliyun-oss-1等',
    `goods_detail`  TEXT          COMMENT '商品详情富文本',
    `sale_times`    INT           DEFAULT 0 COMMENT '委托售卖次数',
    `goods_status`  TINYINT       COMMENT '商品业务状态 1挂卖中 2已抢购待付款 3等待确认付款 4待处理 5委托代卖',
    `entrust_status` TINYINT      NOT NULL DEFAULT 0 COMMENT '委托状态 0未委托 1委托代卖中',
    `audit_status`  TINYINT       NOT NULL DEFAULT 0 COMMENT '审核状态 0无需审核 1待审核 2审核通过 3审核驳回',
    ...
    KEY `idx_entrust_audit` (`entrust_status`, `audit_status`) COMMENT '委托/审核状态筛选联合索引',
...
CREATE TABLE IF NOT EXISTS `t_consign_goods_operate_log` (
    ...
    `from_status`  TINYINT         DEFAULT NULL COMMENT '业务流转前状态(仅operate_type=5有效 1挂卖中 2已抢购待付款 3等待确认付款 4待处理 5委托代卖)',
...
CREATE TABLE IF NOT EXISTS `t_order` (
    ...
    `pay_voucher_platform`  varchar(50)  DEFAULT NULL COMMENT '支付凭证存储平台:local-1/aliyun-oss-1等',
...
CREATE TABLE IF NOT EXISTS `t_rob_order` (
    ...
    `prev_payment_amount`    DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT '【已废弃，列保留】下单时点商品付款金额旧值快照(单价口径)；原为取消链式回滚基数，2026-09-19起新单不再填充，存量值供脏数据恢复脚本参考',
    ...
    `pay_status`             TINYINT       NOT NULL DEFAULT 0 COMMENT '收款回款状态：0未收款/1已收款/2已回款/3无效（与order_status解耦，正向0→1→2不可逆，取消订单时统一置3）',
    `receipt_operate_user_id` BIGINT       DEFAULT NULL COMMENT '确认收款操作人ID（关联sys_user.id，确认收款时写入，取消订单时保留不清空）',
    `receipt_operate_time`   DATETIME      DEFAULT NULL COMMENT '确认收款时间（确认收款动作发生时写入）',
    `payback_operate_user_id` BIGINT       DEFAULT NULL COMMENT '确认回款操作人ID（关联sys_user.id，确认回款时写入，取消订单时保留不清空）',
    `payback_operate_time`   DATETIME      DEFAULT NULL COMMENT '确认回款时间（确认回款动作发生时写入）',
    `is_deleted`             TINYINT(1)    NOT NULL DEFAULT 0 COMMENT '逻辑删除 0未删 1已删',
...
CREATE TABLE IF NOT EXISTS `t_rob_order_operate_log` (
    ...
    `operate_user_name` VARCHAR(64)  DEFAULT NULL COMMENT '操作人名称快照',
    `receipt_amount`    DECIMAL(12,2) NOT NULL DEFAULT 0.00 COMMENT '回款金额：下单事件快照(自购奖金+付款金额×数量)，取消/转移事件存同订单下单事件行同值正数(展示符号由流水查询层决定)',
    `receipt_round_amount` DECIMAL(12,0) NOT NULL DEFAULT 0 COMMENT '回款金额取整值(四舍五入整数)：下单事件对回款金额HALF_UP取整，取消/转移事件复制下单行同值',
    `payment_amount`    DECIMAL(12,2) NOT NULL DEFAULT 0.00 COMMENT '付款金额(本金总额口径)：下单事件快照(商品付款单价×数量)，取消/转移事件存同订单下单事件行同值正数；注意与t_consign_goods.payment_amount单价口径区分',
    `buyer_id`          BIGINT       DEFAULT NULL COMMENT '事件买家ID(下单=下单买家/取消=取消时买家/转移=新买家；存量历史行为NULL由查询层COALESCE兜底)',
    `buyer_name`        VARCHAR(64)  DEFAULT NULL COMMENT '事件买家名称快照(下单/取消/转移正向行买家)',
    `buyer_phone`       VARCHAR(32)  DEFAULT NULL COMMENT '事件买家手机号快照(下单/取消/转移正向行买家)',
    `prev_buyer_id`     BIGINT       DEFAULT NULL COMMENT '转移前买家ID(仅转移事件快照，下单/取消及存量历史行为NULL)',
    `prev_buyer_name`   VARCHAR(64)  DEFAULT NULL COMMENT '转移前买家名称快照(仅转移事件)',
    `prev_buyer_phone`  VARCHAR(32)  DEFAULT NULL COMMENT '转移前买家手机号快照(仅转移事件)',
    `prev_inviter_id`   BIGINT       DEFAULT NULL COMMENT '转移前推荐人ID(仅转移事件快照)',
...
-- =============================================
-- 用户操作审计日志表：sys_user_operate_log
-- =============================================
-- 设计要点：
--   1. 管理端用户写操作(创建/编辑/启停/删除/转老会员)与定时任务系统自动禁用统一落此表
--   2. 定时任务系统操作 operate_user_id=NULL、operate_user_name='SYSTEM'
--   3. 写入走独立事务异步落库(对齐 t_order_operate_log 惯例)，审计链不随业务回滚丢失
--   4. 注：日志表不建外键，避免级联删除丢失审计记录、保留完整历史
